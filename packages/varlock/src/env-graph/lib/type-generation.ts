...
function resolveCsTypeGenerationOptions(options?: TypeGenerationOptions): CsTypeGenerationOptions {
  ...
  const publicOnly = options?.publicOnly === undefined ? false : options.publicOnly;
  ...
  if (typeof publicOnly !== 'boolean') {
    throw new Error('@generateTypes(lang=cs) - `publicOnly` must be a boolean');
  }
...
export async function generateCsTypesSrc(items: Array<TypeGenItemInfo>, options?: TypeGenerationOptions) {
  ...
  // Filter items based on publicOnly setting
  const filteredItems = csOptions.publicOnly
    ? items.filter((info) => !info.isSensitive)
    : items;

  // Fail loudly if publicOnly mode would produce an empty type
  if (csOptions.publicOnly && filteredItems.length === 0) {
    throw new Error('@generateTypes(lang=cs, publicOnly=true) - all items are sensitive; public-only generation would produce an empty type');
  }

  const members = getCsTypeGenMembers(filteredItems);
  ...
  // Only include PropertyBinding class if not publicOnly (since it has IsSensitive property)
  if (!csOptions.publicOnly) {
  ...
  // Only include SensitiveKeys if not publicOnly
  if (!csOptions.publicOnly) {
    csSrc.push('');
