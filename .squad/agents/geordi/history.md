...
- 2026-03-16: Repo-owned temp artifacts now have an honest local home: JS/Vitest suites and the .NET proof harness should create unique directories under `.tmp/` via `packages/utils/src/repo-temp.ts`, while .NET tests should use `packages/dotnet/Varlock.DotNet.Tests/TestPaths.cs`. Keep negative-control cases that must be outside any git repo on OS temp so the behavior under test stays real.
- 2026-03-16: The six-character Copilot/context-mode ignore rule must stay root-anchored (`/[a-z0-9][a-z0-9][a-z0-9][a-z0-9][a-z0-9][a-z0-9]/`). An unanchored pattern silently ignores nested directories like `packages/dotnet/`, which can hide legitimate new .NET source files such as test helpers.
