...
The DX overhaul also now ships focused console siblings under `examples/dotnet-console-*` for direct load, typed config, sensitive handling, Serilog redaction, reload, custom schema path, custom working directory, provider-level environment fallback, optional startup, and injected custom runtime behavior. Those siblings are teaching/proof artifacts for those exact seams only; they do not widen the app-type support matrix beyond the rows below.
