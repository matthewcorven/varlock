...
Each example should prove only its intended seam. Framework examples answer the framework-integration question, while focused console siblings own cross-cutting feature demos like typed binding, Serilog redaction, reload behavior, and other advanced lanes.
...
Those examples prove direct low-level runtime loading, `WebApplicationBuilder.AddVarlock()` and `HostApplicationBuilder.AddVarlock()` integration, startup configuration-provider layering over `appsettings.json`, User Secrets and `local.settings.json` coexistence where documented, legacy non-hosted `net48` bridge usage, Blazor Server startup configuration access, and the Blazor WebAssembly public-only generation boundary. Typed binding, Serilog redaction, reload behavior, and other feature-heavy stories are proven by the focused console siblings and package tests instead of being duplicated inside the framework examples.
...
| ASP.NET Core MVC provider usage | `examples/dotnet-aspnet-mvc-net8/` | `bun run proof:dotnet` ASP.NET provider check | startup-only `WebApplicationBuilder` integration with `appsettings.json` ordering and User Secrets coexistence; reload, typed binding, and Serilog live in focused sibling or test lanes | proven |
| Worker Service / Generic Host usage | `examples/dotnet-worker-net8/` | `bun run proof:dotnet` worker host check | proves `HostApplicationBuilder.AddVarlock()` wiring in a `BackgroundService`; reload semantics remain a separate proof lane | proven |
...
| `dotnet watch` runtime reload behavior | `examples/dotnet-aspnet-mvc/` | `bun run proof:dotnet` watch stability check | proven via `dotnet watch run` with a 6-second stability window; the ASP.NET MVC example starts cleanly and shows zero pathological rebuilds during the observation period | proven |
| Explicit `dotnet build` example flow | `examples/dotnet-console-net8/`, `examples/dotnet-worker-net8/`, `examples/dotnet-aspnet-mvc-net8/`, `examples/dotnet-functions-isolated-net8/`, `examples/dotnet-blazor-server-net8/`, `examples/dotnet-winforms-net48/`, and `examples/dotnet-blazor-wasm-net8-public/` | `bun run proof:dotnet` explicit `dotnet build` check | proves clean compilation for all checked-in examples across `net8.0` (most) and `net48` (WinForms); generated-file loops and IntelliSense observations remain planned | proven |
...
| `IOptions<T>` binding | `examples/dotnet-console-di-options/` | `bun run proof:dotnet` di-options check | proven for the current manual hosted options pattern only; generated-type and convenience-overload stories remain separate proof lanes | proven |
| `IOptionsSnapshot<T>` scoped reload | `examples/dotnet-console-options-snapshot/` | `bun run proof:dotnet` options-snapshot check | proves request-scoped `IOptionsSnapshot<T>` resolution via `CreateScope()` with the manual options mapping pattern; reload-on-change semantics remain a separate proof lane | proven |
| `IOptionsMonitor<T>` long-lived reload | `examples/dotnet-console-options-monitor/` | `bun run proof:dotnet` options-monitor check | proves singleton `IOptionsMonitor<T>` resolution with the manual options mapping pattern; change-notification semantics remain a separate proof lane | proven |
| C# type generation | `examples/dotnet-console-typed-config/` and `examples/dotnet-console-public-only/` | `bun run proof:dotnet` typed-config check, public-only check, and packed `Varlock.MSBuild` package-consumer proof | focused console specimens plus the packed `Varlock.MSBuild` consumer now own the generation claim; framework examples no longer carry MSBuild generation proof | proven |
| Serilog-specific destructuring redaction | `examples/dotnet-console-serilog/` | `bun run proof:dotnet` console Serilog check | proven only for Serilog: the console specimen shows exact, case-sensitive sensitive-key redaction with the literal `[REDACTED]` without implying broader hosting semantics | proven |
...
| Non-default `SchemaPath` configuration | `examples/dotnet-console-custom-schema-path/` | `bun run proof:dotnet` custom-schema-path check | proves only the `AddVarlock(...)` `SchemaPath` override with a checked-in `config/.env.schema` entry point | proven |
| Custom working directory configuration | `examples/dotnet-console-custom-working-dir/` | `bun run proof:dotnet` custom-working-dir check | proves only the `AddVarlock(...)` `WorkingDirectory` override while keeping the default `.env.schema` file name | proven |
| Provider-level `EnvironmentName` propagation | `examples/dotnet-console-environment-name/` | `bun run proof:dotnet` environment-name check | proves that `AddVarlock(...)` forwards `EnvironmentName = "production"` into `VarlockLoadOptions` for runtimes that honor it; CLI-backed env-file resolution from that option alone remains unproven in the example surface | proven |
| `Optional = true` startup behavior | `examples/dotnet-console-optional/` | `bun run proof:dotnet` optional startup check | proves graceful startup with a missing configured entry point while `ReloadOnChange` stays false; later file appearance remains a separate proof path | proven |
| Custom `IVarlockRuntime` injection | `examples/dotnet-console-custom-runtime/` | `bun run proof:dotnet` custom-runtime check | proves `VarlockConfigurationSource.Runtime` injection into the configuration-provider pipeline only; it does not stand in for CLI lookup or bridge-contract proof | proven |
| CLI-bridge coercion surfaced through IConfiguration plus resolved-graph types | `examples/dotnet-console-coercion/` | `bun run proof:dotnet` coercion check | proves IConfiguration string output plus boolean/numeric `VarlockResolvedGraph` value types only; no new binder claim | proven |
| Startup validation failure surfacing | `examples/dotnet-console-validation/` | `bun run proof:dotnet` validation check | example intentionally exits 1 after printing bridge category/message for a missing required key; proves the startup failure surface, not recovery behavior | proven |
| Console public-only generated-type boundary | `examples/dotnet-console-public-only/` | `bun run proof:dotnet` public-only check | proves build-time generated-file filtering only; sensitive property names and sensitive metadata are excluded while `PropertyKeys` metadata remains | proven |
| Local `exec()` resolver seam | `examples/dotnet-console-exec/` | `bun run proof:dotnet` exec check | proven only for a checked-in local `bun -e ...` command seam; not a broader secret-manager claim | proven |
| Schema reference composition | `examples/dotnet-console-composition/` | `bun run proof:dotnet` composition check | proves `${}` reference composition into final configuration values; no broader inheritance or `@extends` claim is made here | proven |
| Manual hosted DI/options pattern | `examples/dotnet-console-di-options/` | `bun run proof:dotnet` di-options check | proves a user-authored manual `IOptionsMonitor<T>` mapping from IConfiguration only; no generated-type specific binder claim | proven |
| Generic hosted `AddVarlock<TConfig>()` convenience overloads | no standalone example required | `dotnet test packages/dotnet/Varlock.DotNet.Tests/Varlock.DotNet.Tests.csproj --filter HostingExtensionsTests` | proves host/web `AddVarlock<TConfig>()` overloads delegate to existing AddVarlock semantics plus standard `Configure<TConfig>(builder.Configuration)` binding | proven |
| Explicit `ExecutablePath` override | `examples/dotnet-console-explicit-executable/` | `bun run proof:dotnet` explicit-executable check | proves a repo-relative explicit CLI path override with local and `PATH` lookup disabled in the specimen | proven |
| `PreventLeaks` metadata surface | `examples/dotnet-console-leak-prevention/` | `bun run proof:dotnet` leak-prevention check | proves `VarlockResolvedGraph.PreventLeaks` plus manual redaction-helper output only; no automatic interception or enforcement is implied | proven |
...
Current repository automation runs `bun run proof:dotnet` on Windows, macOS, and Linux through `.github/workflows/test.yaml`, with Linux also running the full lint/build/test suite. The proof command includes explicit `dotnet build` checks for each checked-in example before the runtime assertions, the first, second, and third console sibling batches, the console redaction-helper proof, the ASP.NET `--options-proof`, `--serilog-proof`, and `--snapshot-proof` paths, worker reload proofs, and generated-file validation for both the console and WASM public-only boundaries. WinForms runtime assertions remain Windows-only; non-Windows lanes keep the honest build-only proof for that example.
...
- focused proof artifacts, not the framework examples themselves, carry the options story: the checked-in DI/options sibling proves the current manual `IOptions<T>` pattern, while dedicated `IOptionsSnapshot<T>` and long-lived `IOptionsMonitor<T>` specimens remain planned
...
- ~~the security-boundary specimen exists and makes the Serilog versus non-Serilog distinction concrete~~ **done:** `bun run proof:dotnet` now exercises the focused console Serilog specimen, the console `--redaction-helper-proof`, and the Blazor WebAssembly public-only boundary with the documented narrow caveats
...
- Example applications demonstrate the focused Serilog specimen and the separate non-Serilog manual helper specimen.
...
- Example apps demonstrate typed access where that behavior is the point of the specimen.
- Example apps demonstrate validation behavior.
- Example apps demonstrate `Optional` behavior where relevant.
- Example apps demonstrate `ReloadOnChange` behavior where a dedicated reload specimen exists.
