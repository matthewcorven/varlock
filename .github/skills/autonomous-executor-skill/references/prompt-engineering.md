# Prompt Engineering for Ralph Task Execution

Guide for generating effective Ralph prompts from task specification markdown files. The orchestrating agent reads this reference when composing the prompt file that Ralph will loop on.
...
## Goal
{Paste the Goal section verbatim from the task specification.}

## Scope

### In Scope
{Paste the In scope bullet list from the task specification.}

### Out of Scope
{Paste the Out scope bullet list from the task specification. Ralph must respect these boundaries.}
...
{Paste the Implementation Notes section from the task specification.}
...
Run: `cd src/web && npx vitest run --reporter=verbose 2>&1 | tee tasks/artifacts/{TASK-ID}/unit/test-results.txt`
OR: `cd src/api && dotnet test DogTrials.Api.Tests --filter "Category={TASK-ID}" -v minimal 2>&1 | tee tasks/artifacts/{TASK-ID}/unit/test-results.txt`
...
Screenshots: Save to `tasks/artifacts/{TASK-ID}/playwright/screenshots/` using naming convention `{NN}-{use-case}-{action}.png`
...
- `tasks/artifacts/{TASK-ID}/unit/test-results.txt` — unit test output
- `tasks/artifacts/{TASK-ID}/playwright/screenshots/` — E2E screenshots (if applicable)
- `tasks/artifacts/{TASK-ID}/playwright/test-results.txt` — E2E test output (if applicable)
- `tasks/artifacts/{TASK-ID}/db/verification-output.txt` — DB verification (if applicable)
- `tasks/artifacts/{TASK-ID}/telemetry/` — telemetry verification (if applicable)

Create artifact directories with: `mkdir -p tasks/artifacts/{TASK-ID}/{unit,playwright/screenshots,db,telemetry}`
...
- **BLOCKED** — unresolvable external issue → writes `../../.tmp/ralph/blocked-TASK-{ID}.json`
- **FAILURE** — exhausted approaches → writes `../../.tmp/ralph/failed-TASK-{ID}.json`
...
1. **Include the full task content** — Ralph's agent sees only the prompt; it has no other context about what to build.
...
4. **Include the "Out of Scope" section** — Prevents the agent from wandering into adjacent tasks.
...
3. **Don't include instructions for multiple tasks in one prompt** — One prompt per task. Use Ralph's tasks mode only if the task itself has explicit sub-tasks.
4. **Don't reference external URLs** — The agent can't browse the web. Include all necessary content inline.
5. **Don't set conflicting goals** — If the task has both "create" and "don't modify" for related files, resolve the conflict before generating the prompt.

## Adapting for Task Types

### API/Backend Tasks

Tasks that touch entities, migrations, endpoints, or background processing. Emphasize:
...
Tasks that touch web components, styles, or page layouts. Emphasize:
...
### Infrastructure/Config Tasks

Tasks that touch build config, orchestration, CI/CD, or Azure infrastructure. Emphasize:
- Configuration file locations
- Environment variable requirements
- Docker/container commands if applicable
- Verification commands that prove the config works

## Example: Generating a Prompt for a UI Config Task

Given a task like `tasks/items/TASK_XXX_Token_Palette_Alignment.md`, the orchestrator would:
...
5. Generate prompt with:
   ...
   - Target values table from the task spec
   - Build verification: `cd src/web && npm run build`
   - Test command: `cd src/web && npx vitest run`
   - Artifact: `tasks/artifacts/TASK-{ID}/unit/test-results.txt`
   - Completion promise
...
| Including 3 tasks in one prompt | Agent loses focus, burns tokens | One prompt per task |
