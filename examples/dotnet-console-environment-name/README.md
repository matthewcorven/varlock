...
This example proves the narrow `EnvironmentName` propagation path for `AddVarlock(...)`. The app injects a tiny `IVarlockRuntime` that reads `VarlockLoadOptions.EnvironmentName` and returns the production variant when the provider supplies `EnvironmentName = "production"`.
