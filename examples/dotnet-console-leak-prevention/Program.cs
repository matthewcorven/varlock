...
builder.AddVarlock(); // 👈 Varlock: surface PreventLeaks metadata and keep the example explicit about its non-enforcing boundary

using var app = builder.Build();

var configuration = app.Services.GetRequiredService<IConfiguration>();
var graph = app.Services.GetRequiredService<VarlockResolvedGraph>();
// PreventLeaks is metadata here; the plaintext secret is still available in-process.
...
// Console output is redacted only because this sample calls the helper explicitly.
